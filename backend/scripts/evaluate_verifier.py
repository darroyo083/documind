"""Run the DocuMind evidence-verifier evaluation harness (PoC 3F-A).
...
2. Real semantic verifier benchmark -- requires explicit opt-in and local
   ...
   - --verifier-model <name> selects the external model explicitly; the future
     verifier milestone decides which model to spend calls on.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "the future verifier milestone decides which model to benchmark.",
    )
    parser.add_argument(
        "--embedding-provider",
